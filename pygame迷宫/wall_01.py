...
class Maze:
    ...
    def nei(self):
        ...
        for move in self.moves:
            if 0 < self.x + move[0] < 20 and 0 < self.y + move[1] < 20:
...
class Wall_2:
    def __init__(self):
        ...
        road1.clear()
        n = 0
        maze = [[Maze(x, y) for x in range(21)] for y in range(21)]
        for i in range(21):
            for j in range(21):
                if i % 2 == 1 and j % 2 == 1:
                    # maze[i][j].maze=1
...
wookbook=load_workbook('墙01.xlsx')
wooksheet=wookbook['Sheet1']
row=1
for i in road3:
    a,b=i
    # print(a,b)
    wooksheet.cell(row=row, column=1, value=a)
    wooksheet.cell(row=row, column=2, value=b)
    row+=1

wookbook.save('墙01.xlsx')

Wall_2()
from openpyxl import load_workbook
wookbook=load_workbook('墙02.xlsx')
wooksheet=wookbook['Sheet1']
row=1
for i in road3:
    a,b=i
