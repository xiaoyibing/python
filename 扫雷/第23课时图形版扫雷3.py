# 任务4：图形版扫雷3，要求具有官版基础版扫雷的全部功能（包含右键功能，雷数功能）
import random
import time
import turtle as t
import os
...
class Lei:
    def __init__(self):
        ...
        self.p2.goto(-400, 150)
        self.p2.write('雷数：%s' % num_lei, font=('宋体', 40, 'normal'))
        t.onscreenclick(self.lei, btn=3)
    ...
    def lei(self, x, y):
        ...
        if 0 <= x % 25 <= 20 and 0 <= y % 25 <= 20 and 0 <= x <= 250 and -225 <= y <= 25:
            ...
            j = int((y // (-25)) + 1)
            self.p1.goto(i * 25, j * (-25))
            self.p1.write('雷', font=('宋体', 20, 'normal'))
            if self.mains[i][j].write_in == '*':
                num_lei -= 1
                self.p2.clear()
                self.p2.write('雷数：%s' % num_lei, font=('宋体', 40, 'normal'))
